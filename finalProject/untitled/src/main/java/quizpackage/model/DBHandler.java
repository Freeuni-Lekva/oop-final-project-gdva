...
public class DBHandler {
    ...
    public void addQuiz(String quizTitle, String order, String alignment, String answerType, int creatorId, String description) {
        ...
        System.out.println("newID: " + newID + " for quiz: " + quizTitle);
        try{
            if(containsQuiz(quizTitle)){
                System.out.println("contains quiz");
                return ;
            }
    ...
    public void removeQuiz(String title){
        try{
            ResultSet resultSet = connection.createStatement().executeQuery("select * from quizzes where title = '" + title + "'");
            resultSet.next();
            int quiz_id = resultSet.getInt("id");
            clearQuizHistory(quiz_id);
            connection.createStatement().
